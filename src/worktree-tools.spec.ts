...
describe('worktree lifecycle tools', () => {
	...
	test('rejects unsafe names and unregistered existing directories', () => {
		const root = repository();
		expect(enterWorktree(root, {name: '../escape'}).content).toContain('safe');
...
test('removes a clean merged worktree and optionally its branch', async () => {
	...
	Bun.spawnSync(['git', 'commit', '-qm', 'merged change'], {cwd: entered.cwd});
