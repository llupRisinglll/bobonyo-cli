...
describe('system prompt styles', () => {
	test('default returns the built-in prompt untouched', () => {
		expect(resolveSystemPrompt('default', DEFAULT_PROMPT)).toBe(DEFAULT_PROMPT);
	});
	...
	test('presets carry the REAL tool prompts (opencode/codex/claude-code)', () => {
		...
		expect(SYSTEM_PROMPT_PRESETS.opencode).toMatch(
		...
		expect(SYSTEM_PROMPT_PRESETS.claudecode).toMatch(
	...
	test('custom falls back to the default until SYSTEM.md exists', () => {
		...
		expect(resolveSystemPrompt('custom', DEFAULT_PROMPT)).toBe(DEFAULT_PROMPT);
	});
	...
	test('selecting custom SEEDS SYSTEM.md and it is then loaded', () => {
		...
		expect(resolveSystemPrompt('custom', DEFAULT_PROMPT)).toContain('BoboNyo');
	});
});
